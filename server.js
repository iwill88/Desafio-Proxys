...
if (parsedArgs.m==="CLUSTER" && cluster.isPrimary){
  ...
  const connectedServer = httpServer.listen(parsedArgs.p, () => {
    console.log(`Servidor Http con Websockets escuchando en el puerto ${parsedArgs.p} - PID(${process.pid}) - FYH(${new Date().toLocaleString()})`)
  })
